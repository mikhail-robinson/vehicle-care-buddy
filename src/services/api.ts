...
export async function createCar(carData: Omit<Car, 'id' | 'created_at' | 'updated_at'>): Promise<Car> {
  try {
    const response = await fetch(`${API_BASE_URL}/cars`, {
      method: 'POST',
      headers: {
...
export async function fetchServiceHistory(carId?: number): Promise<ServiceHistoryEntry[]> {
  try {
    const url = carId 
...
export async function createServiceEntry(
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/cars/${carId}/service_history_entries`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export async function fetchServiceEntryDetails(entryId: number): Promise<ServiceHistoryEntry> {
  try {
    const response = await fetch(`${API_BASE_URL}/service_history_entries/${entryId}`);
    if (!response.ok) {
      const errorData: ApiError = await response.json();
      throw new Error(errorData.error || 'Failed to fetch service entry details');
    }
    return await response.json();
  } catch (error) {
    console.error(`Error fetching service entry details for ID ${entryId}:`, error);
    throw error;
  }
}
